  ...
  <nav id="nav" class="navbar navbar-expand-lg navbar-dark navbar-custom col">
    ...
    >
      ...
      <ul class="navbar-nav ml-auto">
        <li v-for="item of menuItems.pages" :key="item.id" class="nav-item">
          <nuxt-link
            v-if="isRus"
            class="nav-link"
...
export default {
  ...
  methods: {
    startPosition() {
      ...
      if (this.currentSlug.name === "index") document.location.reload();
    },
    setRuLanguage() {
  ...
  watch: {
    currentSlug: function(val) {
      if (val.name !== "index") {
        this.$bus.$emit("removeZIndexVisible");
      } else {
        this.$bus.$emit("changeVisibility");
      }
      if (
