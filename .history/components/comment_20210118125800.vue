<template>
    <div class="comment-wrap d-flex flex-column-reverse pl-4 py-2">
        <button
            @click="toggleIsFull"
            class="btn btn-custom btn-outline-success"
            v-if="!isFull && this.comments.length > 4"
        >
        ...
        <div
            class="my-2"
            v-for="item in arrayToShow"
            :key="item.id"
            @click="$emit('toggleCommentConfig', item)"
        >
            <div class="comment">
                <div class="d-flex">
                    <p style="color: #0069d9; white-space: nowrap" class="mr-2">
                        {{ item.author.name }}:
                    </p>
                    <p>{{ item.text }}</p>
                </div>
            ...
            <span v-if="currentUserName === item.author.name" class="inline"
                >...</span
            >
            <div>
                <small class="pl-4">{{
                    transformDate(item.created_at)[0]
                }}</small>
                <small class="pl-4">{{
                    transformDate(item.created_at)[1].split(".")[0]
                }}</small>
            </div>
        </div>
    </div>
        ...
        getComment() {
            this.$axios.get(`news_comments/${this.currentPost}`).then((res) => {
                ...
                this.comments.length < 4
                    ...
                    : (this.cutComment = this.comments.slice(
    ...
    watch: {
        comments: function (newcomments, oldcomments) {
            if (newcomments.length > oldcomments.length) {
                ...
                this.$store.commit(
                    "SET_USER_NAME",
                    newcomments[newcomments.length - 1].author.name
                );
            }
        },
    },
