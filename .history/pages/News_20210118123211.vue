<template>
    <div v-if="news" class="news-wrapper d-flex">
        <img
            v-if="getWindowWidth <= 1281"
            class="bg"
            src="/news/1280News.png"
            alt="big"
        />
        <img
            v-else-if="getWindowWidth > 1280 && getWindowWidth < 1681"
            class="bg"
            id="bg"
            src="/news/1680News.png"
            alt="big"
        />
        <img
            v-else-if="
                getWindowWidth > 1681 &&
                getWindowWidth < 1921 &&
                getWindowHeight < 1090
            "
            class="bg"
            src="/news/1920News.jpg"
            alt="big"
        />
        <img
            v-else-if="
                getWindowWidth > 1681 &&
                getWindowWidth < 1921 &&
                getWindowHeight > 1091
            "
            class="bg"
            src="/news/19201News.jpg"
            alt="big"
        />
        <img
            v-else-if="getWindowWidth >= 1921 && getWindowWidth < 2800"
            class="bg"
            src="/news/2048News.png"
            alt="big"
        />
        ...
        <main class="news pb-1">
            ...
            >
                <div class="news-item__header">
                    <h3>
                        {{ language === "ru" ? item.ru_name : item.ua_name }}
                    </h3>
                ...
                <div class="news-item__body">
                    ...
                    <div
                        class="likes-container d-flex justify-content-between pt-2 pb-3"
                    >
                        ...
                        <span
                            ...
                            @click="showAllComment(item.id)"
                        >
                            {{
                                $options.filters.toUSD(language, "Комментарии")
                            }}: {{ item.comments_count }}
                        </span>
                    </div>
                ...
                <div
                    class="news-item__footer likes-container d-flex justify-content-between py-2"
                >
                    <v-popover v-if="!isShowModal">
                        ...
                        >
                            <img
                                ...
                                alt="like"
                            />
                            <span class="ml-1 caps">{{
                                $options.filters.toUSD(language, "Нравится")
                            }}</span>
                        </div>
                        <template
                            v-if="!isCookie"
                        ...
                        >
                            ...
                            <social
                                @emitOpenFormModal="showModal('like', item.id)"
                            />
                        </template>
                    </v-popover>
                    ...
                    <v-popover v-if="!isShowModal">
                        <div
                            @click="showCommentField(item.id)"
                            tooltip-target
                            style="width: 200px"
                ...
                <transition name="fade">
                    <comment
                        ...
                        @toggleCommentConfig="toggleCommentConfig"
                        :tok="tok"
        ...
        >
            <login
                v-if="isLogin && !isCommentConfigVisible"
                :target="target"
                :id="currentId"
...
import CommentConfigPopup from "../components/commentConfigPopup";
...
export default {
    ...
    computed: {
        ...
        ...mapGetters(["language", "currentUserName"]),
    },
    methods: {
        toggleCommentConfig(comment) {
            console.log(comment.author.name, this.currentUserName, this.tok);
            if (this.tok && this.currentUserName === comment.author.name) {
                this.currentComment = comment;
                this.isCommentConfigVisible = true;
        ...
        closeModal() {
            ...
            this.isCommentConfigVisible = false;
        },
        closeModalAndUpdate() {
            this.closeModal();
            this.updateComments();
        ...
        cuttedText(val) {
            this.language === "ru"
                ? (val = val.ru_description)
                : (val = val.ua_description);
            if (val.length > 100) {
                return (
                    val.slice(0, 100) +
                    "..." +
                    `<a class="moreLink">${this.$options.filters.toUSD(
                        this.language,
                        "ещё"
                    )}</a>`
                );
        ...
        addComment(data) {
            ...
            if (this.isCookie) {
                if (this.file) {
                    ...
                    this.$axios
                        .$post(`add_comment/${this.currentPost}`, form_data, {
                            crossdomain: true,
                            mode: "no-cors",
                            dataType: "json",
                            headers: {
                    ...
                    this.$axios
                        .$post(`add_comment/${this.currentPost}`, {
                            image: null,
                            text: data,
                            token: this.tok,
                            withCredentials: true,
        ...
        sendEditComment(data) {
            ...
            if (this.isCookie) {
                if (this.file) {
                    ...
                    this.$axios
                        .$post(
                            `update_comment/${this.currentComment.id}`,
                            form_data,
                            {
                                crossdomain: true,
                                mode: "no-cors",
                    ...
                    this.$axios
                        .$post(`update_comment/${this.currentComment.id}`, {
                            image: null,
                            text: data,
                            token: this.tok,
                            withCredentials: true,
    ...
    mounted() {
        if (process.client) {
            this.$bus.$on("sendEditComment", this.sendEditComment);
            this.$bus.$on("emitToImageSave", this.imageSave);

            this.$bus.$on("addLike", this.addLike);
